...
public class DesignHashMap {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < action.length; i++) {
            ...
            System.out.println("action " + action[i] + " input " + input[i][0]);
...
class MyHashMap {
    ...
    public MyHashMap() {
        this.myHashMap = new int[0][0];
    }
    
    public void put(int key, int value) {
        int Len = myHashMap.length;
        int check = 0;
        int[] KeyValue = {key,value};

        for (int i = 0; i < Len; i++) {
            if (myHashMap[i][0] == key) {
                myHashMap[i] = KeyValue;
                check++;
            } 
        }
        
        if (check == 0){
            int[][] HoldMap = new int[Len+1][2];
            for (int i = 0; i < Len; i++) {
                HoldMap[i] = myHashMap[i];
            }

            HoldMap[Len] = KeyValue;
