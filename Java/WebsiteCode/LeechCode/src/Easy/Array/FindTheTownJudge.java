...
class FindTheTownJudge_Solution {
    public int findJudge(int n, int[][] trust) {
        HashMap<Integer,Integer> trustPersons = new HashMap<>();

        for (int i = 0; i < trust.length; i++) {
            trustPersons.put(trust[i][1],trustPersons.getOrDefault(trust[i][1], 0)+1);
        }

        for (int person : trustPersons.keySet()) {
            if (trustPersons.get(person) == n-1) {return validJudge(trust, person);}
        }

        return -1;
    }

    public int validJudge(int[][] trust, int judge) {
        for (int[] per : trust) {
            if (per[0] == judge) {return -1;}
        }
