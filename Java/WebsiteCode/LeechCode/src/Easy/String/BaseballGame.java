...
/**
 * <a class="mr-2 text-label-1 dark:text-dark-label-1 hover:text-label-1 dark:hover:text-dark-label-1 text-lg font-medium" href="/problems/baseball-game/">682.Baseball Game</a>
 * 
 * <div class="px-5 pt-4"><div class="flex"></div><div class="xFUwe" data-track-load="description_content"><p>You are keeping the scores for a baseball game with strange rules. At the beginning of the game, you start with an empty record.</p>
...
public class BaseballGame {
    public static void main(String[] args) {
        ...
        BaseballGame_Solution res = new BaseballGame_Solution();

        System.out.println(res.calPoints(operations));
    }
}

class BaseballGame_Solution {
    // 2 ms
    // 40.3 MB
    public int calPoints(String[] operations) {
        ...
        for (int i = 0; i < operations.length; i++) {
            ...
            switch (operations[i]) {
                ...
                case "D":
                    records.add(records.get(index)*2);
                    break;

                case "+":
                    records.add(records.get(index) + records.get(index-1));
                    break;
    ...
    public int calPoints(String[] operations) {
        ...
        for (int i = 0,j=0; i < operations.length; i++) {
            switch (operations[i]) {
                case "C":
                    j --;
                    sum -= Integer.parseInt(operations[j]);
                    break;
                
                case "D":
                    operations[j] = String.valueOf(Integer.parseInt(operations[j-1])*2);
                    sum += Integer.parseInt(operations[j]);
                    j++;
                    break;

                case "+":
                    operations[j] = String.valueOf(Integer.parseInt(operations[j-1]) + Integer.parseInt(operations[j-2]));
                    sum += Integer.parseInt(operations[j]);
                    j++;
                    break;
...
class BaseballGame_Solution3 {
    ...
    public int calPoints(String[] operations) {
        ...
        for (int i = 0; i < operations.length-skip; i++) {
