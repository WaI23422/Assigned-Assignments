...
class MaximumScoreAfterSplittingAString_Solution {
    public int maxScore(String s) {
        int score = 0, zeroLeft = 0, oneRight = 0;
        char[] sChars= s.toCharArray();

        for (int i = 0; i < sChars.length; i++) {
            if (sChars[i] - '0' == 1 && i != 0) {oneRight++;}
            if (sChars[i] - '0' == 0 && i != sChars.length-1) {zeroLeft++;}
            
            if (zeroLeft > oneRight) {
                score += zeroLeft;
                oneRight = 0; zeroLeft = 0;
            }
        }

        return score+oneRight;
    }
}