...
class LargestSubstringBetweenTwoEqualCharacters_Solution {
    public int maxLengthBetweenEqualCharacters(String s) {
        ...
        int total = 0, max = Integer.MIN_VALUE, temp;

        for (char i : sChars) {charArr[i-'a']++;}
        for (int i : charArr) {
            if (i >= 2) {total++;}
        }
        ...
        for (int i = 0; i < charArr.length; i++) {
            if (charArr[i] >= 2) {
                temp = indexLast('a'+i, s) - s.indexOf('a'+i) - 1;
    ...
    public int indexLast(int c, String s){
        int idx = 0;

        for (int i = s.length()-1; i >= 0; i--) {
            if (s.charAt(i) == c) {idx = i; return idx;}       
        }
