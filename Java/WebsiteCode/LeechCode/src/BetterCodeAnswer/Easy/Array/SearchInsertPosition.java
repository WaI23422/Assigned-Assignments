...
/**
 * <a class="mr-2 text-label-1 dark:text-dark-label-1 hover:text-label-1 dark:hover:text-dark-label-1 text-lg font-medium" href="/problems/search-insert-position/">35.Search Insert Position</a>
 * 
...
public class SearchInsertPosition {
    public static void main(String[] args) {
        ...
        for (int[][] test : tests) {
            int[] nums = test[0];
            int target = test[1][0];
            System.out.println(new SearchInsertPosition_Solution().searchInsert(nums,target));
        }
    }
}

// @see Easy.Array.SearchInsertPosition.java
